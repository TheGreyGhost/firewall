...
int main(int argc, char **argv)
{
	...
	h = nflog_open();
	if (!h) {
	    perror("error during nflog_open()\n");
		exit(1);
	}
	...
	if (nflog_bind_pf(h, AF_INET) < 0) {
		perror("error during nflog_bind_pf()\n");
		exit(1);
	}
	printf("binding this socket to group 0\n");
	qh = nflog_bind_group(h, 0);
	if (!qh) {
		perror("no handle for group 0\n");
		exit(1);
	}
	...
	if (!qh100) {
		perror("no handle for group 100\n");
		exit(1);
	}

	printf("setting copy_packet mode\n");
	if (nflog_set_mode(qh, NFULNL_COPY_PACKET, 0xffff) < 0) {
		perror("can't set packet copy mode\n");
		exit(1);
	}
